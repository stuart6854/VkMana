...
namespace VkMana::SamplesApp
{
    namespace Utils
    {
        ...
        auto LoadGLTFMaterials(tinygltf::Model& gltfModel, Renderer& renderer, const std::vector<ImageHandle>& images) -> std::vector<MaterialHandle>
        {
                ...
                for(auto i = 0; i < mesh.primitives.size(); ++i)
                {
                    ...
                    { // Vertices
                        ...
                        if(primitive.attributes.contains("TANGENT"))
                        {
                            ...
                            bufferTangents
                                = reinterpret_cast<const float*>(&(model.buffers[tangentView.buffer].data[tangentAccessor.byteOffset + tangentView.byteOffset])
                                );
                        }
                    ...
                    { // Indices
                        ...
                        case TINYGLTF_PARAMETER_TYPE_UNSIGNED_BYTE:
                        {
                            ...
                            memcpy(buf.data(), &buffer.data[accessor.byteOffset + bufferView.byteOffset], accessor.count * sizeof(uint8_t));
                            for(size_t index = 0; index < accessor.count; index++)
                            {
                                triangles.push_back(buf[index] /* + submesh.VertexOffset*/);
                            }
